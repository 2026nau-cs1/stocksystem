...
import type { StockListItem, WatchlistGroup, WatchlistItem } from '@/types';
...
export default function WatchlistView() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (groupsResponse.success) {
        ...
        setSelectedGroup((current) => current ?? groupsResponse.data[0]?.id ?? null);
      }
    ...
    await groupAction.run(
      async () => {
        ...
        setGroups((current) => {
          ...
          setSelectedGroup((selected) => (selected === id ? nextGroups[0]?.id ?? null : selected));
          return nextGroups;
        });
  ...
  const handleAddItem = async (symbol: string, name: string) => {
    if (!selectedGroup) {
    ...
    await itemAction.run(
      async () => {
        const response = await watchlistApi.addItem(selectedGroup, symbol, name);
  ...
  const selectedGroupName = groups.find((group) => group.id === selectedGroup)?.name ?? '自选股';
                ...
                >
                  <span
                    className={`text-sm font-mono ${
                      selectedGroup === group.id ? 'text-[#16A34A]' : 'text-[var(--app-text)]'
                    }`}
                  >
                    {group.name}
                  </span>
        ...
        <div className="rounded-xl border border-[var(--app-border)] bg-[var(--app-surface)] p-4 shadow-sm lg:col-span-3">
          <div className="mb-4 flex items-center justify-between">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-[var(--app-text)]">
              ...
              <span className="ml-2 font-normal text-[var(--app-muted)]">({groupItems.length} 只)</span>
            </h3>
            {selectedGroup ? (
              <button
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs font-mono text-[var(--app-muted)]">{item.symbol}</div>
                    </div>
                    <div className="flex items-center gap-4">
                      ...
                      <button
                        onClick={() => void handleRemoveItem(item.id, item.name)}
