...
export default function PortfolioView() {
  ...
  const handleStockSelect = (symbol: string) => {
    ...
    if (!stock) {
      setForm((current) => ({ ...current, symbol: '', name: '', currentPrice: '' }));
      return;
    }
    ...
    await submitAction.run(
      async () => {
        if (editId) {
          ...
          setHoldings((current) =>
            current.map((holding) => (holding.id === editId ? response.data : holding))
          );
  ...
  const totalCost = holdings.reduce(
    (sum, holding) => sum + parseFloat(holding.shares) * parseFloat(holding.costPrice),
    0
  );
  const totalValue = holdings.reduce(
    (sum, holding) => sum + parseFloat(holding.shares) * parseFloat(holding.currentPrice),
    0
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        {[
          { label: '总市值', value: formatCurrency(totalValue), color: 'text-[var(--app-text)]' },
          { label: '总成本', value: formatCurrency(totalCost), color: 'text-[var(--app-muted)]' },
          {
            label: '浮动盈亏',
          ...
          <div key={card.label} className="rounded-xl border border-[var(--app-border)] bg-[var(--app-surface)] p-4 shadow-sm">
            <div className="mb-1 text-xs font-mono text-[var(--app-muted)]">{card.label}</div>
      ...
      {showForm ? (
        <div className="rounded-xl border border-[var(--app-border)] bg-[var(--app-surface)] p-5 shadow-sm">
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            <div>
              <label className="mb-1 block text-xs font-mono text-[var(--app-muted)]">股票</label>
              <select
                value={form.symbol}
            ...
            <div>
              <label className="mb-1 block text-xs font-mono text-[var(--app-muted)]">持股数量</label>
              <input
                type="number"
                value={form.shares}
            ...
            <div>
              <label className="mb-1 block text-xs font-mono text-[var(--app-muted)]">成本价（元）</label>
              <input
                type="number"
            ...
            <div>
              <label className="mb-1 block text-xs font-mono text-[var(--app-muted)]">现价（元）</label>
              <input
                type="number"
                value={form.currentPrice}
                onChange={(event) =>
                  setForm((current) => ({ ...current, currentPrice: event.target.value }))
                }
          ...
          <div className="mt-4 flex gap-3">
            ...
            >
              {submitAction.isRunning ? '保存中...' : editId ? '更新持仓' : '添加持仓'}
            </button>
            <button
              onClick={resetForm}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[var(--app-border)]">
                  {['股票', '持股数量', '成本价', '现价', '市值', '浮动盈亏', '收益率', '占比', '操作'].map(
                    (header) => (
                      <th
                        key={header}
              ...
              <tbody className="divide-y divide-[var(--app-border)]">
                {holdings.map((holding, index) => {
                  ...
                  return (
                    <tr key={holding.id} className="group transition-colors hover:bg-[var(--app-soft-hover)]">
                      ...
                      <td className="px-4 py-3 text-sm font-mono text-[var(--app-text)]">{cost.toFixed(2)}</td>
                      <td className="px-4 py-3 text-sm font-mono text-[var(--app-text)]">
                        {current.toFixed(2)}
                      </td>
                      <td className="px-4 py-3 text-sm font-mono text-[var(--app-text)]">
                      ...
                      <td
                        className={`px-4 py-3 text-sm font-mono ${
                          isPositive ? 'text-[#16A34A]' : 'text-red-500'
                        }`}
                      >
                        {isPositive ? '+' : ''}
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-mono text-[var(--app-muted)]">{weight.toFixed(1)}%</span>
                        </div>
                      </td>
                      <td className="px-4 py-3">
      ...
      {holdings.length > 0 ? (
        <div className="rounded-xl border border-[var(--app-border)] bg-[var(--app-surface)] p-5 shadow-sm">
          ...
          <div className="space-y-3">
            {holdings.map((holding, index) => {
              const value = parseFloat(holding.shares) * parseFloat(holding.currentPrice);
              const weight = totalValue > 0 ? (value / totalValue) * 100 : 0;
              const pnlPct =
                parseFloat(holding.costPrice) > 0
                  ? ((parseFloat(holding.currentPrice) - parseFloat(holding.costPrice)) /
              ...
              return (
                <div key={holding.id}>
                  <div className="mb-1 flex items-center justify-between">
                    <span className="text-xs font-mono text-[var(--app-text)]">{holding.name}</span>
                    <div className="flex items-center gap-3">
                      ...
                      <span
                        className={`text-xs font-mono ${
                          pnlPct >= 0 ? 'text-[#16A34A]' : 'text-red-500'
                        }`}
                      >
                        {pnlPct >= 0 ? '+' : ''}
                        {pnlPct.toFixed(1)}%
