...
import type { Alert, AlertConditionType, AlertHistoryItem, AlertStatus } from '@/types';
...
export default function AlertsView() {
    ...
    await submitAction.run(
      async () => {
        if (editId) {
          ...
          setAlerts((current) =>
            current.map((alert) => (alert.id === editId ? response.data : alert))
          );
  ...
  const handleToggleStatus = async (alert: Alert) => {
    ...
    await updateAction.run(
      async () => {
        const response = await alertsApi.updateAlert(alert.id, { status: nextStatus });
      ...
      {showForm ? (
        <div className="rounded-xl border border-[var(--app-border)] bg-[var(--app-surface)] p-5 shadow-sm">
          ...
          <div className="mb-4 grid grid-cols-1 gap-3 md:grid-cols-2">
            ...
            <div>
              <label className="mb-1 block text-xs font-mono text-[var(--app-muted)]">触发条件</label>
              <select
            ...
            <div>
              <label className="mb-1 block text-xs font-mono text-[var(--app-muted)]">
                {form.conditionType.includes('change') ? '涨跌幅（%）' : '价格（元）'}
              </label>
              <input
                type="number"
                ...
                placeholder={form.conditionType.includes('change') ? '例如 5' : '例如 28.80'}
            ...
            <div>
              <label className="mb-2 block text-xs font-mono text-[var(--app-muted)]">通知方式</label>
              <div className="flex items-center gap-3">
                {[
                  ...
                  <label key={key} className="flex cursor-pointer items-center gap-1.5">
                    ...
                    />
                    <span className="text-xs font-mono text-[var(--app-text)]">{label}</span>
                  </label>
                ))}
              </div>
            </div>
          ...
          <div className="flex gap-3">
            ...
            >
              {submitAction.isRunning ? '保存中...' : editId ? '更新预警' : '创建预警'}
            </button>
            <button
              onClick={resetForm}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[var(--app-border)]">
                    {['股票', '触发条件', '通知方式', '触发次数', '状态', '操作'].map((header) => (
                      <th
                        key={header}
                ...
                <tbody className="divide-y divide-[var(--app-border)]">
                  {alerts.map((alert) => {
                    ...
                    return (
                      <tr key={alert.id} className="group transition-colors hover:bg-[var(--app-soft-hover)]">
                        <td className="px-4 py-3">
                          <div className="text-sm font-mono font-semibold text-[var(--app-text)]">
                            {alert.stockName}
                          </div>
                          <div className="text-xs font-mono text-[var(--app-muted)]">{alert.symbol}</div>
                        </td>
                        <td className="px-4 py-3">
                          <span
                          ...
                          >
                            {CONDITION_LABELS[alert.conditionType]} {isChange ? '' : '¥'}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="ml-2 text-xs font-mono text-[var(--app-muted)]">{item.symbol}</span>
                    </div>
                  </div>
