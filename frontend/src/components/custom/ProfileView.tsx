...
export default function ProfileView() {
  ...
  const loadProfileData = async () => {
    ...
    try {
      ...
      if (meResponse.success) {
        ...
        const nextTheme = (meResponse.data.theme as 'dark' | 'light') || 'light';
        setTheme(nextTheme);
        ...
        setRefreshRate(String(meResponse.data.refreshRate || 3));
      }
      ...
      if (alertsResponse.success) {
        setActiveAlertCount(
          alertsResponse.data.filter((alert) => alert.status === 'active').length
        );
      }
    } finally {
      setLoading(false);
  ...
  const handleFeedbackSubmit = async () => {
    ...
    if (!trimmedFeedback) {
      await feedbackAction.run(async () => {
  ...
  const handleSaveSettings = async () => {
    await settingsAction.run(
      async () => {
        ...
        setTheme((response.data.theme as 'dark' | 'light') || theme);
        applyTheme((response.data.theme as 'dark' | 'light') || theme);
      },
      {
  ...
  const handleThemeChange = async (nextTheme: 'dark' | 'light') => {
    ...
    await settingsAction.run(
      async () => {
        ...
        setUser(response.data);
        setTheme((response.data.theme as 'dark' | 'light') || nextTheme);
        applyTheme((response.data.theme as 'dark' | 'light') || nextTheme);
      },
      {
            ...
            {loading ? (
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-base font-semibold text-[var(--app-text)]">{user.name}</div>
                  <div className="truncate font-mono text-sm text-[var(--app-muted)]">{user.email}</div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              >
                ...
                <div className="mt-1 font-mono text-xs text-[var(--app-muted)]">{stat.label}</div>
              </div>
            ))}
          </div>
          ...
          <div className="rounded-xl border border-[var(--app-border)] bg-[var(--app-surface)] p-5">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between gap-3">
                <label className="text-sm text-[var(--app-text)]">界面主题</label>
                <select
                  value={theme}
                  onChange={(event) => {
                    void handleThemeChange(event.target.value as 'dark' | 'light');
                  }}
              ...
              <div className="flex items-center justify-between gap-3">
                <label className="text-sm text-[var(--app-text)]">行情刷新频率</label>
                <select
                  value={refreshRate}
                  onChange={(event) => setRefreshRate(event.target.value)}
                  className="rounded-lg border border-[var(--app-border)] bg-[var(--app-bg)] px-3 py-1.5 font-mono text-xs text-[var(--app-text)] outline-none transition-colors focus:border-[#16A34A]"
                >
                  <option value="1">实时（1 秒）</option>
