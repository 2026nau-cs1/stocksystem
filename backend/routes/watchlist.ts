...
router.get('/groups', authenticateJWT, async (req: AuthRequest, res: Response) => {
  try {
...
// Create group
router.post('/groups', authenticateJWT, async (req: AuthRequest, res: Response) => {
  try {
    const userId = getAuthenticatedUserId(req);
    ...
    const msg = getErrorMessage(error, 'Failed to create group');
    res.status(400).json({ success: false, message: msg });
  }
});

// Delete group
router.delete('/groups/:id', authenticateJWT, async (req: AuthRequest, res: Response) => {
  try {
    const userId = getAuthenticatedUserId(req);
...
// Get items by group
router.get('/groups/:groupId/items', authenticateJWT, async (req: AuthRequest, res: Response) => {
  try {
    const userId = getAuthenticatedUserId(req);
...
router.post('/items', authenticateJWT, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const msg = getErrorMessage(error, 'Failed to add item');
    res.status(400).json({ success: false, message: msg });
  }
});

// Remove item
router.delete('/items/:id', authenticateJWT, async (req: AuthRequest, res: Response) => {
  try {
    const userId = getAuthenticatedUserId(req);
    const id = req.params.id as string;
